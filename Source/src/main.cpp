...
int main(int argv, char** args) {
	...
	// Cria a janela
	gWindow = SDL_CreateWindow("Freecell", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_RESIZABLE);
	if (gWindow == NULL) {
...
//		SDL_SetWindowFullscreen(gWindow, SDL_WINDOW_FULLSCREEN_DESKTOP);
	...
	for (int i = 0; i < 4; i++) {
		p_d[i].setPosition({window_size.x/2 + 45 + 90 * i, 50});
	...
	for (int i = 0; i < 8; i++) {
		...
		p_i[i].setPosition({window_size.x/2 - 385 + 100 * i, 200});
