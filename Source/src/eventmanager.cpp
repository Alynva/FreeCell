	...
	for (int i = 1; i < this->stacks.getSize(); i++) { // Inicia em 1 pois o primeiro � a pilha que persegue o mouse
		...
		if (nodeStack->value->getSize()) {
			...
			if (found && nodeStack->value->canBeMoved(&lastNodeCard->value)) {
				...
				for (int j = 0; j <= nodeStack->value->getSize(); !afterThan ? j++ : j) { // Se encontrar, n�o incrementa o j pois ele ser� retirado da pilha, portanto o pr�ximo ser� o atual depois de ser retirado
