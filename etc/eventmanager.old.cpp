...
void EventManager::mouseLeftDown() {
	...
	PilhaInteligente* mouse_stack = this->stacks.peek()->dir->value;

	// Alynva: Implementaï¿½ï¿½o do [] continua nï¿½o funcionando
	Node<PilhaInteligente*>* nodeStack = this->stacks.peek()->dir;
...
//	mouse_stack->clear();
	...
	for (int i = 1; i < this->stacks.getSize(); i++) { // Inicia em 1 pois o primeiro ï¿½ a pilha que persegue o mouse
		...
		if (nodeStack->value->getSize()) {
			...
			if (found && nodeStack->value->canBeMoved(&lastNodeCard->value)) {
				Node<Carta>* first_node_card_mouse = mouse_stack->peek()->dir;
	...
	while (!stack_temp.isEmpty()) {
		...
		SDL_Log("%c", test ? 's' : 'n');
		if (!test) original_stack->push(card_temp, test); // Alynva: o PilhaDefinitiva.push() está sempre retornando true, suspeito que é por que o this->peek() não existe, portanto não é possível executar getValue()
		SDL_Log("%c", test ? 's' : 'n');
	}
}
...
void EventManager::mouseMove() {
	...
	for (int i = 1; i < this->stacks.getSize(); i++) { // Inicia em 1 pois o primeiro ï¿½ a pilha que persegue o mouse
		...
		if (nodeStack->value->getSize()) {
			bool isMovable = false;
			bool found = false;
			Node<Carta>* lastNodeCard = new Node<Carta>();
			Node<Carta>* nodeCard = nodeStack->value->peek()->dir;
			for (int j = 0; j < nodeStack->value->getSize(); j++) {
				if (nodeCard->value.isInside(mouse_pos)) {
				SDL_Log("->> %i%c", nodeCard->value.getValue(), nodeCard->value.getSuit());
					lastNodeCard = nodeCard;
					found = true;
				}
	...
	Node<Carta>* nodeCard = mouse_stack->peek();
